...
import { fontWeight } from 'components/constant'
...
const ServiceCardWrap = styled.div`
  ...
  .service-bg-color {
    ...
    .service-wrap {
      ...
      background: url(${require("../../../assets/img/service1.png").default}) no-repeat right/contain;
      padding: 13px;
      width: 100%;
      overflow: hidden;
      .service-img {
...
const CardDiv = styled.div`
  ...
  &.out_of_stock {
    .card-wrap {
      position: relative;
      &:before {
        content: '${(props => props?.out_of_stock)}';
        width: 100%;
        height: 40px;
        position: absolute;
        top: 0;
      ...
      &:after {
        ...
        width: 100%;
        height: 100%;
        background-color: ${rgba(colors.whisper, .75)};
      }
    }
  }
`
...
const CategoryBgImg = ({ data, border_radius, ...props }) => {
  ...
  return (
    <BackgroundData data={data} border_radius={border_radius} title={props?.title} className="category-bg-img">
      <CustomPlaceholder {...CategoryBgSettings} />
      {props.children}
    </BackgroundData>
  )
}
...
const ServiceBgImg = ({ data, border_radius, ...props }) => {
  return (
    <BackgroundData data={data} border_radius={border_radius} title={props?.title} className="service-bg-img">
      <CustomPlaceholder {...ServiceBgSettings} />
      {props.children}
    </BackgroundData>
  )
}
...
const PromoBgImg = ({ data, border_radius, ...props }) => {
  ...
  return (
    <BackgroundData data={data} border_radius={border_radius} title={props?.title}>
      <CustomPlaceholder {...PromoBgSettings} />
      {props.children}
    </BackgroundData>
  )
}
